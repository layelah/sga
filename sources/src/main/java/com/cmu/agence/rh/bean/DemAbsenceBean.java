...
public class DemAbsenceBean extends BaseCRUDBean {
    ...
    public String getNomCreateur(){
      // int idUser =  sess.srvCurrentUserId();
       
       return getAuth().getUser().getAgent().getNomComplet();
    }
    ...
    public String enregistrerPermission() {
        ...
        try {
            ...
            m_obj.getAbsence().setNomCreateur(getAuth().getUser().getAgent().getNomComplet());
...
// Date identique et depart = aprem et fin = matin
        
        if((m_obj.getAbsence().getDateDebutAbsence().equals(m_obj.getAbsence().getDateFinAbsence())) && ((this.getMoment_deb().equalsIgnoreCase("apres_midi") && this.getMoment_fin().equalsIgnoreCase("matin")))){
            JsfUtil.addErrorMessage("Erreur sur la saisie des conges");
            yaErreur = true;
        }
    ...
    public boolean checkDuSoldeConges(DemandePermissionAbsence m_obj) {
        boolean yaErreur = false;
        
       
        
        if (scApres.getNbCongesReliquat() < 0 || scApres.getNbCongesEchus() < 0) {
            JsfUtil.addErrorMessage("date_debut", "Le nombre de jour de congés demandé dépasse votre reliquat");
            yaErreur = true;
        }
        
//        if (scAvant.getNbCongesEchus() < m_obj.getAbsence().getNbJourDeductible()) {
//            JsfUtil.addErrorMessage("nb_jour_absence", "Le nombre de jour de congés demandé dépasse le nombre de jour de congés échus");
//            yaErreur = true;
//        }
        return yaErreur;        
    }
    ...
    //AbsenceExceptionnelle absExcep;
       ...
       // System.out.println("Nom createur demande: "+this.getNomCreateur());
        ...
        //COntrole sur le compteur conges
        
        if(checkDuSoldeConges(m_obj)){
            return null;
        }
        ...
        try {
            ...
            //Mise à jour des heures de depart et d'arrivée de l'absence
              ...
              System.out.println("Date de debut: "+ m_obj.getAbsence().getDateDebutAbsence());
              System.out.println("Date de fin: "+ m_obj.getAbsence().getDateFinAbsence());
        ...
        /**************************************************************************/
            ...
            if (this.getFile().getFileName().equalsIgnoreCase("")){
                ...
                m_obj.getAbsence().setCheminFileUploaded("/Users/issathiam/NetBeansProjects/SGA/sources/src/main/webapp/resources/piecesJointes");
                 // m_obj.getAbsence().setCheminFileUploaded("/home/srvdebian/deploiement/SGA_V3/resources/piecesJointes");
                String chemin = m_obj.getAbsence().getCheminFileUploaded();
                String nomFichier = this.save(this.getFile(), chemin);
                m_obj.getAbsence().setFileUploaded(nomFichier);
            }
...
//            }else{
 ...
 //           }
    ...
    public void ajaxCalculDateNbj(Absence absence) {
        ...
        //Date differente mais depart = matin et fin = aprem
        
        if(!(absence.getDateDebutAbsence().equals(absence.getDateFinAbsence())) && ((this.getMoment_deb().equalsIgnoreCase("matin") && this.getMoment_fin().equalsIgnoreCase("apres_midi")))){
             m_obj.getAbsence().setNbJourAbsence( JIDate.nbJour(absence.getDateDebutAbsence(), absence.getDateFinAbsence())  + 1.00 );
             ...
             m_obj.getAbsence().setNbJourDeductible(JIDate.nbJourOuvrable(absence.getDateDebutAbsence(), absence.getDateFinAbsence()) - (nbJourFerie ));
        }
        
         //Date differente mais depart = aprem et fin = matin
        
        if(!(absence.getDateDebutAbsence().equals(absence.getDateFinAbsence())) && ((this.getMoment_deb().equalsIgnoreCase("apres_midi") && this.getMoment_fin().equalsIgnoreCase("matin")))){
             m_obj.getAbsence().setNbJourAbsence( JIDate.nbJour(absence.getDateDebutAbsence(), absence.getDateFinAbsence()) );
             m_obj.getAbsence().setNbJourOuvrable(JIDate.nbJourOuvrable(absence.getDateDebutAbsence(), absence.getDateFinAbsence()) - (nbJourFerie) - 1.00);
             m_obj.getAbsence().setNbJourDeductible(JIDate.nbJourOuvrable(absence.getDateDebutAbsence(), absence.getDateFinAbsence()) - (nbJourFerie ) - 1.00);
        }
        
        //Date differente mais [(depart = aprem et fin = aprem ) OU (depart = matin et fin = matin )]
        
        if(!(absence.getDateDebutAbsence().equals(absence.getDateFinAbsence())) && (estDemiJournee())){
             m_obj.getAbsence().setNbJourAbsence( JIDate.nbJour(absence.getDateDebutAbsence(), absence.getDateFinAbsence()) );
             m_obj.getAbsence().setNbJourOuvrable(JIDate.nbJourOuvrable(absence.getDateDebutAbsence(), absence.getDateFinAbsence()) + 0.50 - (nbJourFerie) - 1.00);
             m_obj.getAbsence().setNbJourDeductible(JIDate.nbJourOuvrable(absence.getDateDebutAbsence(), absence.getDateFinAbsence()) - (nbJourFerie ) + 0.50 - 1.00);
        }
        
       //Date identique mais [(depart = aprem et fin = aprem ) OU (depart = matin et fin = matin )]
        if((absence.getDateDebutAbsence().equals(absence.getDateFinAbsence())) && (estDemiJournee())){
             m_obj.getAbsence().setNbJourAbsence( 0.50 );
             m_obj.getAbsence().setNbJourOuvrable( 0.50 );
             m_obj.getAbsence().setNbJourDeductible( 0.50 );
        }
        
         //Date identique mais [(depart = matin et fin = aprem]
        if((absence.getDateDebutAbsence().equals(absence.getDateFinAbsence())) && ((this.getMoment_deb().equalsIgnoreCase("matin") && this.getMoment_fin().equalsIgnoreCase("apres_midi")))){
             m_obj.getAbsence().setNbJourAbsence( 1.00 );
             m_obj.getAbsence().setNbJourOuvrable( 1.00 );
             m_obj.getAbsence().setNbJourDeductible( 1.00 );
        }
...
//    }
    ...
    public void ajaxCalculDateNbjExcep(Absence absence) {
        ...
        if(!(absence.getDateDebutAbsence().equals(absence.getDateFinAbsence())) && ((this.getMoment_deb().equalsIgnoreCase("matin") && this.getMoment_fin().equalsIgnoreCase("apres_midi")))){
             m_obj.getAbsence().setNbJourAbsence( JIDate.nbJour(absence.getDateDebutAbsence(), absence.getDateFinAbsence())  + 1.00 );
             m_obj.getAbsence().setNbJourOuvrable(JIDate.nbJourOuvrable(absence.getDateDebutAbsence(), absence.getDateFinAbsence())  - (nbJourFerie));
             m_obj.getAbsence().setNbJourNonDeductible(JIDate.nbJourOuvrable(absence.getDateDebutAbsence(), absence.getDateFinAbsence()) - (nbJourFerie ));
        }
        ...
        if(!(absence.getDateDebutAbsence().equals(absence.getDateFinAbsence())) && ((this.getMoment_deb().equalsIgnoreCase("apres_midi") && this.getMoment_fin().equalsIgnoreCase("matin")))){
             m_obj.getAbsence().setNbJourAbsence( JIDate.nbJour(absence.getDateDebutAbsence(), absence.getDateFinAbsence()) );
             m_obj.getAbsence().setNbJourOuvrable(JIDate.nbJourOuvrable(absence.getDateDebutAbsence(), absence.getDateFinAbsence()) - (nbJourFerie) - 1.00);
             m_obj.getAbsence().setNbJourNonDeductible(JIDate.nbJourOuvrable(absence.getDateDebutAbsence(), absence.getDateFinAbsence()) - (nbJourFerie ) - 1.00);
        }
        ...
        if(!(absence.getDateDebutAbsence().equals(absence.getDateFinAbsence())) && (estDemiJournee())){
             m_obj.getAbsence().setNbJourAbsence( JIDate.nbJour(absence.getDateDebutAbsence(), absence.getDateFinAbsence()) );
             m_obj.getAbsence().setNbJourOuvrable(JIDate.nbJourOuvrable(absence.getDateDebutAbsence(), absence.getDateFinAbsence()) + 0.50 - (nbJourFerie) - 1.00);
             m_obj.getAbsence().setNbJourNonDeductible(JIDate.nbJourOuvrable(absence.getDateDebutAbsence(), absence.getDateFinAbsence()) - (nbJourFerie ) + 0.50 - 1.00);
        }
       ...
       //Date identique mais [(depart = aprem et fin = aprem ) OU (depart = matin et fin = matin )]
        if((absence.getDateDebutAbsence().equals(absence.getDateFinAbsence())) && (estDemiJournee())){
             m_obj.getAbsence().setNbJourAbsence( 0.50 );
             m_obj.getAbsence().setNbJourOuvrable( 0.50 );
             m_obj.getAbsence().setNbJourNonDeductible( 0.50 );
        }
        ...
        if((absence.getDateDebutAbsence().equals(absence.getDateFinAbsence())) && ((this.getMoment_deb().equalsIgnoreCase("matin") && this.getMoment_fin().equalsIgnoreCase("apres_midi")))){
             m_obj.getAbsence().setNbJourAbsence( 1.00 );
             m_obj.getAbsence().setNbJourOuvrable( 1.00 );
             m_obj.getAbsence().setNbJourNonDeductible( 1.00 );
        }
...
//        }
    ...
    //Liste des conges exceptionnels
    public List getCongesExcepList() throws EchecSelectException {
        return srv.listAbsExcep();
    }
    //Liste des demandes validees
    public List getDemAcceptees() throws EchecSelectException {
        return srv.listDemAcceptees();
    }
    //Liste des demandes refusees
    public List getDemRefusees() throws EchecSelectException {
        return srv.listDemRefusees();
    }
    //Liste des demandes en cours
    public List getDemandesEnCours() throws EchecSelectException {
        return srv.listDemEnCours();
    }
    ...
    //Liste des demandes archivees
    public List getDemArchivees() throws EchecSelectException {
        return srv.listDemArchivees();
    }
...
//    public Agent agentCreateurDemande() throws EchecSelectException {
//        //DemandePermissionAbsence demAbs = (DemandePermissionAbsence) obj;
//        return srv.nomCreateur(getAuth().getUser().getAgent().getId());
//    }
    ...
    public boolean estDemiJournee(){
        return ((this.getMoment_deb().equalsIgnoreCase("matin") && this.getMoment_fin().equalsIgnoreCase("matin")) || (this.getMoment_deb().equalsIgnoreCase("apres_midi") && this.getMoment_fin().equalsIgnoreCase("apres_midi")));
    }
