...
//        scAvant = new SoldeConges();
    ...
    public TableValeurService getTvSrv() {
        return tvSrv;
    }

    public void setTvSrv(TableValeurService tvSrv) {
        this.tvSrv = tvSrv;
    }

    public SoldeCongesService getScSrv() {
        return scSrv;
    }

    public void setScSrv(SoldeCongesService scSrv) {
        this.scSrv = scSrv;
    }

    public AbsenceService getSrv() {
        return srv;
    }

    public void setSrv(AbsenceService srv) {
        this.srv = srv;
    }
    ...
    public List getAbsencesRegularisees() throws EchecSelectException {
        return scSrv.listAbsRegularisees();
    }
...
//    }
    ...
    public void ajaxCalculDateNbj() { // Absence absence
    //SoldeConges m_obj = (DemandePermissionAbsence) obj;
    //Absence absence = new Absence(agt);
    Integer nbJourFerie = srv.getNbJourFerie(absence.getDateDebutAbsence(), absence.getDateFinAbsence());  
    SoldeConges scAgt = scSrv.getSoldeConges(absence.getAgent().getId());
...
//    scAgt.setNbCongesEchus(scAgt.getNbCongesEchus() - absence.getNbJourDeductible());
//    scAgt.setNbJourPris(scAgt.getNbJourPris() + absence.getNbJourDeductible());
//    scAgt.setNbCongesReliquat(scAgt.getNbCongesReliquat()- absence.getNbJourDeductible());
    
//    try {
//            scAgt = (SoldeConges)scSrv.save(scAgt);
//        } catch (CMUServiceException ex) {
//            Logger.getLogger(SoldeCongeBean.class.getName()).log(Level.SEVERE, null, ex);
//        }
    ...
    public String enregistrerAbsence() {
        ...
        try {
            TableValeur typeAbsence = tvSrv.getRef(TableValeurTypeEnum.TypeAbsence, AbsenceEnum.Regularisation.toString());
            ...
            absence.setNomCreateur(getAuth().getUser().getAgent().getNomComplet());
            
            // Sauvegarde de l'absence
            absence = (Absence) srv.save(absence);

            
            // Mise a jour du solde de conges
            SoldeConges sc1 = scSrv.getSoldeConges(absence.getAgent().getId());
                    sc1.setNbCongesEchus(sc1.getNbCongesEchus() - absence.getNbJourDeductible());
                    System.out.println("Nb jour Deductible: "+ absence.getNbJourDeductible());
                    System.out.println("Nb jour Pris avnt: "+ sc1.getNbJourPris());
                    sc1.setNbJourPris(sc1.getNbJourPris() + absence.getNbJourDeductible());
                    
                    sc1.setNbCongesReliquat(sc1.getNbCongesReliquat() - absence.getNbJourDeductible());             
                    //sc1.setNbJourPrisDeductible(sc1.getNbJourPrisDeductible() + absence.getNbJourDeductible());
                    sc1 = (SoldeConges)scSrv.save(sc1);     
                    System.out.println("Nb jour Pris apres: "+ sc1.getNbJourPris());
                    System.out.println("Nouveau Solde");
        System.out.println("Reliquat de l'agent: "+ sc1.getNbCongesReliquat());
        System.out.println("Echus de l'agent: "+ sc1.getNbCongesEchus());
        System.out.println("Pris de l'agent: "+ sc1.getNbJourPris());
